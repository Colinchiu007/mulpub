...
```bash
# 只改根 package.json，并按规则 bump
pnpm version:bump patch     # 2.3.53 -> 2.3.54（修复级）
pnpm version:bump minor     # 涨功能级（0.x 下破坏性变更也走这里）
pnpm version:bump major     # 仅当已 ≥1.0.0 时涨主版本
...
1. 合并所有待发 PR 到 main。
2. 在 main 上 `pnpm version:bump <级别>`，提交版本变更。
3. 打 tag：`git tag vX.Y.Z` 并推送。
4. CI（`build.yml`）检出 tag 后执行 `scripts/sync-version.mjs`，用根版本覆盖 `apps/desktop`，
   electron-builder 据此生成安装包（artifactName 模板使用 `package.json` 的 `version`）。
5. 把 CHANGELOG 顶部这批 `## [未发布]` 收口为 `## [vX.Y.Z] - <日期>`。
...
- `scripts/bump-version.mjs` —— 按级别 bump 根版本并同步 desktop。
- `scripts/sync-version.test.mjs` —— 零依赖单测（`node scripts/sync-version.test.mjs`）。
- `scripts/hooks/pre-commit` —— 提交前自动跑 `sync-version.mjs` 并对齐 `apps/desktop/package.json`。
- `.github/workflows/build.yml` —— 发布构建时调用 `sync-version.mjs`。
