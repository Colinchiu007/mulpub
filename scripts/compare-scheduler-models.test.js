...
describe('durationTolerance — 容差 = max(绝对下限, 比例 × 期望耗时)', () => {
  it('短用例保持绝对下限，不因引入比例而被放宽', () => {
    // 1500ms 期望：比例项 1500*0.10=150 < 下限 1500 → 仍取 1500
    assert.equal(durationTolerance(1500), 1500)
    assert.equal(durationTolerance(1000), 1500)
    assert.equal(durationTolerance(14999), 1500)
  })

  it('长用例获得与量级成比例的余量', () => {
    assert.equal(durationTolerance(21000), 2100)
    assert.equal(durationTolerance(9000), 1500) // 900 < 1500，下限仍主导
    assert.equal(durationTolerance(30000), 3000)
  })
  ...
  it('容差必须由「期望值」而非「实测值」驱动，否则回归会撑大自己的容差', () => {
    // 若实现误用 real 作分母：real=31000 → 容差 3100，+10s 回归反而通过（自证式绿灯）
    const expected = 21000
    const tolerance = durationTolerance(expected)
    assert.equal(tolerance, expected * PARITY_TOLERANCE_RATIO)
    assert.ok(
