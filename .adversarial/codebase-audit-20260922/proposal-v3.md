...
- **P0 关闭前置——泄露面排查清单（全部勾选方可关闭问题 1/2）**：
  ...
  - [ ] **[v3]** 服务端配置双钥/宽限期过渡（并行接受新旧公钥、旧钥设到期日），存量客户端升级到安全阈值达标记为轮换完成前置（证据：双钥配置 + 升级率报表）；
