...
8. **【定级：P0（v-final 晋升，原列 P1）】systemd 单元引用未定义变量 + root 运行**
   - **[v3 更正]** 机制订正：systemd `Environment=` 不做 `${}` 展开，进程收到字面量 `${PO_SECRET_KEY}` 作 JWT 秘密——是仓库可复算的确定性公开常量，`get_jwt_secret` 恰好放行，外部可伪造 admin 令牌，**应升为 P0**；修复用 `EnvironmentFile=` 注入真实随机值，并在启动校验中额外拒绝含 `${`/`$(` 的字面量。`routers/env.py:82-92` 的 JWT 对齐检查在本进程内恒返回 `unknown` 属误导性死代码。 **[v3 订正]** 该检查对未配置场景返回 `unknown` 并带 detail 说明且 `test_env_api.py:43-58` 锁定，属跨服务诊断端点的预期行为，非死代码；改为"当前部署单元未注入变量致恒 unknown，属部署配置问题"，处置为在 orchestrator/trendscope 环境补跑或移除该端点。
...
- **超大文件**：- **[v3 验收]** 脆弱等待改造须给出"条件轮询+上限+超时原因"及一个验收用例；Python 侧 `model_preset_service.py`/`prompt_eval_service.py` 引入与 lint `max-lines` 对等的最小行数门禁（新代码阻断、存量挂账）；`flutter-skill-bridge` 处置 deadline=下个发版周期末，判据输出入 CHANGELOG。
...
- **第一批（安全应急，P0 全部 + P1 中的 3/4/6/7/8）**：签名私钥出库+强制轮换（1）、弱密钥闸门+启动期危险组合拒绝（2）、decrypt_key 修正（3，**须先于 4**）、加密主密钥 fail-closed+历史密文掩码降级预案（4，预发布先验，防启动阻断）、CORS 收敛入会话安全矩阵（6）、test_provider_connection SSRF 守卫（7）、systemd 单元变量/运行用户修正（8）。 **[v3]** 8 因 JWT 秘密变公开常量升为第一批内 P0 处置；轮换（1）须附双钥/宽限期过渡方案。**[v-final]** 本批预算仅覆盖代码改动＋单机 pytest/打包验证（约 1-2 天）；下方泄露排查清单与客户端升级阈值属问题 1/2/8 的跨环境关闭前置，单列时间线，不计入本批可交付物。回归以 `cd ops-center/backend && pytest` 门禁；交付物含启动失败应急/回滚指引。
...
- **AGENTS.md QM-2 合同覆盖矩阵（v-final 补记，回应 I6）**——本报告结论与仓库权威安全基线逐条对照：
  ...
  | IPC file URL canonical 合同 | partial（问题 14 覆盖 sender 守卫，未单列 realpath 目录边界回归）|
  ...
  说明：out-of-scope 项复用下方"触发节点/归属"机制登记，非免责声明。
