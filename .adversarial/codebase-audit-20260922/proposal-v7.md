...
> **[v6]** 第 5 轮收口：critique-v5 提 3 条轻量一致性/清晰度项（证据路径前缀、[v3订正] 合并遗留冗余括号、问题2/8 共享密钥机制交叉引用），minScore=8.0 触及收敛阈值，本 v6 全部解决，逐条 Critical 轨迹 4→1→0→0→0，残留清零。

> **[v7·终版]** 第 6 轮（末轮）收口：critique-v6 提 2 条轻量一致性/清晰度项（IPC 计数口径不可复算、问题8 root User= 修复缺细节），minScore=8.0，本 v7 全部解决。六轮 Critical 轨迹 4→1→0→0→0→0→0，评审意见累计 35 条（接受 33、拒绝 1、部分接受 1），r4-r6 十四轮全接受零拒绝，残留清零——报告已达"零问题"收敛终态，v7 为最终定稿。
...
2. **JWT 弱密钥闸门形同虚设 + 跨服务共享密钥**
   - `ops-center/backend/config.py:56-61` 的 `get_jwt_secret` 仅精确匹配拒绝固定串 `dev-secret-change-in-production`；`dev-secret-key-for-local-testing-2026`、口令 `admin123` 均能通过。`ops-center/deploy/ops-center.service:11-14` 又把 `OPS_JWT_SECRET` 与 platform-orchestrator 的 `PO_SECRET_KEY` 绑成同一把（代码层 Stage -1.8 已解耦，部署层重新耦合）。**[v6 交叉引用]** 机制与部署现实以问题 8 的 [v5 裁定] 为准（二者共享同一证据 `ops-center/deploy/ops-center.service:11-14`）：systemd `Environment=` 不展开 `${}`，进程实际收到未展开字面量 `${PO_SECRET_KEY}`（仓库可复算的公开常量）而非 orchestrator 真实密钥；故本条"共享密钥"的准确性质是**模板层未展开字面量堆叠**，其 P0 严重度与攻击面应按"公开常量可被任意人复算伪造 admin 令牌"口径理解（详见问题 8），而非"攻击者已持有真实跨服务凭据"的独立读法。
...
6. **CORS 全开 + credentials**
   - `main.py:74-80`，触发值 `.env:4` 与 `ops-center/deploy/ops-center.service:14`（`OPS_CORS_ORIGINS=*`）。生产收敛为白名单域名，启动期对危险组合告警。**[v2]** 与 15 整合为会话安全矩阵：`credentials=true` 时 `allow_origins` 禁止 `*` 且启动期直接拒绝该组合；白名单精确匹配（子域需显式列出）；15 的 Cookie 迁移必须同步变更 CORS/CSRF/CSP 并互相引用验收。
...
14. **IPC `withSenderCheck` 覆盖不一致**
    - **[v7 口径修正]** 原"约 215/336"经第 6 轮独立复算不可再现（非测试调用站点约 400、唯一 channel 约 385、withSenderCheck 约 319，336 与任何可文档化口径均不吻合），精确守卫覆盖率待 `ipc-guard-count.js` 建后按文档化口径重算；本条严重度与攻击面结论（大量 handler 无 sender 校验）不受计数口径影响；`logs:clear`（删全部日志）、`feedback:submit`（可打包日志外发）及 aggregation/analytics/misc/platform/auto-pipeline/ops-center-sync 等无来源校验。写/破坏性/可外发 handler 统一补守卫；CI 增加"handler 必须声明 guarded 或豁免"静态检查防漂移。**[v2]** 豁免治理：豁免清单集中单文件（显式业务理由+风险等级+到期日/移除条件），豁免下限=只读/非破坏/非外发；CI 既校验 handler 声明完备，也校验豁免条目变更需审批；豁免命中记审计日志。 **[v3/v5]** 计划新建 `scripts/ipc-guard-count.js`（**待建·尚未落地**，全仓当前无此脚本）按文档化口径（channel + 守卫归属，注明含/排除 `.test.js`）精确解析全量 `ipcMain.handle`，随第三批 PR 交付并挂 CI **比例式覆盖率断言**（"守卫覆盖率 ≥ 阈值"，不锚定不可复算的绝对值）；经核验或 CI 挂载前不得宣称该守卫已生效；订正 `license-access-control.js:31` 为功能开关非 sender 守卫，不计入守卫覆盖。
...
- 本次审查为"全仓模式统计 + 高价值文件抽样深读"，未逐行通读全部约 600+ 生产文件。置信度分层（v2 量化）：密钥字面量、shell=True/v-html 等结论来自**全量正则统计，高置信**；IPC 守卫覆盖（原"约 215/336"第 6 轮复算不可再现，**口径待固化·非高置信**，详见问题 14 [v7 口径修正]）；P2 超大文件（CreateView.vue 5589 行等）内部细节**未穷尽逐行**，第三/四批执行时补抽样覆盖。
