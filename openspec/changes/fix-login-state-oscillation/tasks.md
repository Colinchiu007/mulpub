...
- **3.4 未做**：没有为「凭证落盘的 active 固化不经 `loginStatusTransition`」补显式锁死断言。现有护栏只是间接的（`saveCapturedAccount` 用例断言它调用 `persistLoginState`）。留作跟进项，不要在评审时被当成已覆盖。
