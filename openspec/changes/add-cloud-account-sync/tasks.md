...
- [ ] 6.5 QM-1 本地完整打包验证（改了 `apps/desktop/electron/`，必须在干净状态跑 `electron-builder --win --dir` + asar 清单 + require 链 + 启动 8 秒无 stderr）。
...
- [ ] 6.8 `.quality-gates.md` 自检清单与评审记录。
- [ ] 7.3 运维文档：KMS 生产实现要求、005 迁移与「先迁移后发 API」（`assertReady()` 现对未跑 005 的存量库 fail-closed）、feature flag 打开顺序；生产 ECS 发布与 `production-smoke` 均**未执行**。
