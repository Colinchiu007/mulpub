...
- OQ1【wontfix·2026-09-23，10.2】：全量 kit 是否包含非采纳版（同 prompt 中间迭代）以供对比浏览？默认**不包含**（只导采纳版，`iterationCount` 记录迭代数）；如需再开 delta。处置：维持不包含——采纳版已覆盖交付需要，中间迭代仅以 iterationCount 计数暴露（实现见 fetch-hell-grind-kit.py 去重采纳规则）。
- OQ2【deferred·2026-09-23，10.2】：D9 的会话级批量授权是否值得做？待逐批确认实测痛感后定。处置：暂缓——逐批确认前端合同已含"累计已确认预算"缓解项，146 批为极限场景而常规子集出片批次数小；实测痛感反馈强烈再开"本会话免确认"开关 delta。
- OQ3【resolved·2026-09-23，10.2】：参考图目录（images/）是否随全量导入扩充？处置：维持精选 36 张——参考图服务剧本套用场景，全量图片（万级）收益低体积大；扩充需另立图片预算任务。
...
- L3 的"全片"验收基准：以 Hell Grind 采纳版全集（口径修正：6,558 镜，~~2,795~~ 已被 1.3 对账推翻，见上）中用户勾选子集经 manifest 合成可播放 final.mp4，时长与所选镜 durationSec 之和一致（9.3 冒烟实证：混合 10 段 compose，final.mp4 94.58s ≈ ΣdurationSec）。
