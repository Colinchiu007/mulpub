...
| `<中文品牌名>` | 该参考产品的中文名称 |
| `<signer-host>` | 第三方远程签名服务域名（`qianming.` + 全拼小写 + `.cn`）；2026-09-15 签名本地化收口后仅作为抖音验证期对比通道（环境变量 `MP_SIGNER_BASE`）的说明性出现 |
...
| `参考<中文品牌名>` | `参考同类产品` | 避免「参考」+「参考产品」叠词（替换后二次修正） |
| `<中文品牌名>` | `参考产品` | 其余全部中文语境 |
...
### 3.1 行为变化（两阶段：#1837 保留端点 → 2026-09-15 收口本地化）

`packages/api-publish-engine/src/signer.js`：

- **#1837**：`SIGNER_BASE` 收敛为「可覆盖的环境变量 + 默认值」，注释去品牌化；当时判定「属发布链路硬依赖，端点不可移除」。
- **2026-09-15 收口（依赖面复核后本地化）**：签名全部本地化，不再依赖第三方远程服务——
  - **快手 `__NS_sig3`**：本地计算（`MD5(api_ph|body)`，`api_ph` 取自登录 cookie），适配器补传 cookie。⚠️ 复核发现此前适配器调 `getKuaishouSignature(path, postData)` **未传 cookie**，本地兜底实际恒产出空签名——即"远程失败即本地兜底"从未真正生效过；
  - **小红书 / 百家号 / 头条**：远程函数移除（小红书适配器本就直用本地签名、百家号适配器走自身上传链、头条从未实现，三者均无生产调用方）；
  - **抖音 `_signature`**：默认纯本地（本地实现为「浏览器参数 + 占位签名」，真实有效性待真机发布验证）；设置环境变量 `MP_SIGNER_BASE` 后走「远程优先、本地兜底」的验证对比通道。
- **依赖面修正**：此前记载「百家号签名无本地回退属硬依赖」经核实不成立——`getBaijiahaoSignature` 无任何生产调用方；真实依赖面只有抖音 + 快手，且均可本地化。该域名是「远程优先、本地兜底」的优化路径而非硬依赖。
- **遗留**：`<signer-host>` 字面量在仓库中的出现已收敛到验证开关的说明性文档；真机验证通过后移除抖音远程分支即可实现代码零出现。
...
④ 字节级补漏（latin1 保真通道）
   兜底「含 NUL 字节/超大」被当二进制跳过的文本文件；排除二进制扩展名
        │
⑤ 后置修正
   占位符还原（签名域名）、叠词修正（「参考」+「参考产品」→ 参考同类产品）、
   aria-label 语义修正（→ 主导航）
        │
⑥ 残留校验 + 编码安全校验 + 索引完整性校验
        │
...
| 项 | 说明 | 处置建议 |
| --- | --- | --- |
| 第三方签名域名 | `<signer-host>` 曾保留于 `signer.js`（默认端点）与文档；**2026-09-15 收口后签名全部本地化**，域名仅存于抖音验证开关（`MP_SIGNER_BASE`）的使用说明与历史文档 | 真机发布验证抖音/快手本地签名通过率后，移除验证开关即可实现仓库零出现；验证不通过则需提取参考产品签名 JS 本地执行（路 A） |
