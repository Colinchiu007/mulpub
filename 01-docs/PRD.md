...
| `containerSelector` | String | `[data-testid="yixiaoer-workspace"]` | 必须能被 `document.querySelector` 命中 | 未命中时 `console.warn` 且不注册监听，浮标保持隐藏，不抛异常 |
...
1. 用户在内容区产生滚动
2. 主滚动容器 `.yixiaoer-workspace` 以**捕获阶段**收到 `scroll` 事件（`scroll` 不冒泡，仅捕获阶段能同时拿到主容器与任意嵌套 `overflow:auto` 子容器的滚动）
...
| 层级 | 容器 | 举例 | 解析方式 |
|------|------|------|---------|
| 主容器 | `.yixiaoer-workspace` | Accounts.vue、HotTopics.vue | 挂载时 `querySelector` 定位 + 捕获阶段监听 |
...
- [ ] 主导航中不再有「设置」入口
- [ ] `YixiaoerSidebar.test.js`(9) / `YixiaoerModuleNav.test.js`(5) / `ProfileMenu.test.js`(12) 全绿
- [ ] `check-locale-sync.js --cjk / --keys` 通过，无新增硬编码中文
- [ ] CI 全绿（含像素视觉门禁）后再合并
