...
| 2026-09-23 | W1 §6 桌面集成后端：图文 API 发布分流 + 发布方式记录 | `apps/desktop/electron/services/publisher-router.js` `ApiPublisher.publish` 按 `article.video_path` 分流：图文（无 video_path）跳过 ffprobe、taskData 不含 video 字段并透传 images/author（修复 §4.4 百家号 article-only 时桌面抛「缺少视频文件路径」）；视频维持横版强校验。`ApiPublisher`→`mode:'api'`、`RpaVmPublisher` 成功新补 `mode:'dom'`，随 `task.result` 写入 history 作为 §6.1 发布方式徽标（api/dom/降级）数据源。publisher-router.test.js +4 例（图文不探测/无 video 字段/mode:api、images+author 透传、draft 透传、RPA mode:dom），文件 57 测绿；phase4-events/publish/publish-history 回归 47 测绿；ESLint 无 error。待办：§6.1 徽标渲染 + `publish.api.*` locale（需活体视觉验收） | 本 PR；PRD §12.9 |
| 2026-09-23 | W1 §6.1 前端：发布方式徽标三态 + publish.api.* i18n | `PublishHistory.vue` 记录卡 `.record-delivery` 新增「发布方式」徽标，数据源 `record.result.mode`（`deliveryModeValue`/`deliveryModeLabel`/`deliveryModeHint`）：api→「API 直连」蓝、dom→「RPA 浏览器」紫、fallback→「降级发布」琥珀（§5 降级返回 mode:fallback 后启用）；无 mode 不渲染（旧记录向后兼容）。i18n 新增 `publish.api.{modeApi,modeDom,modeFallback,modeApiHint,modeDomHint,modeFallbackHint}`（zh/en 成对）。PublishHistory.test.js +3 例（api/dom/无 mode），文件 28 测绿；i18n.test.js+model-providers-copy 22 测绿；ESLint 无 error；Gate 12 PASS。待办：详情分片历史（随 §7）。 | 本 PR；PRD §12.10 |
| 2026-09-23 | W1 §6.1 前端：发布详情分片增强（发布方式/作品ID/链接） | `PublishHistory.vue` 记录详情弹窗 `.record-detail-grid` 在「发布模式」行后条件渲染三行：发布方式（复用 `deliveryModeLabel`，仅 `result.mode`∈{api,dom,fallback}）、作品 ID（`result.postId`）、作品链接（`result.url`，`<a target="_blank" rel="noopener">` 外链，`data-testid=detail-link`）；helper `resultValue(record,key)` 仅接受 string/number 转字符串，其余空串→对应行 v-if 隐藏（旧记录/RPA 历史向后兼容）。i18n `historyPage.{detailDeliveryMode,detailPostId,detailLink}`（zh/en 成对）。PublishHistory.test.js +2 例（带 result 渲染三行+链接 href/rel、无 result 不渲染），文件 30 测绿；i18n 20 测绿；ESLint 无 error；Gate 12 PASS。多平台子结果完整分片列表待 `result.subResults` 落地扩展（随 §7）。 | 本 PR；PRD §12.11 |
| 2026-09-23 | W1 §5.4 桌面风控挂起信号生产端 | 引擎已有 `createRiskSuspender`/`publish-mode-runner` 风控分类，但桌面 `publisher-router` 直连 Publisher、失败仅 `throw Error` 不经 `publishWithMode`，风控不被识别。新增 `apps/desktop/electron/services/publish-risk.js` `isRiskBlocked(task.error)`（RISK_RE 与引擎同义、非串/空 fail-safe）；`phase4-events.js` `task:failed` 内命中则额外 `webContents.send('publish:risk-hold', {platform, accountId: task.article?.accountId||null, taskId, error})`（仅窗口存活时）。纯识别、无自动恢复（合规：风控即停不换号）。publish-risk.test.js 3 例 + phase4-events.test.js +2 例（发/不发），electron 6 测绿；ESLint 无 error；Gate 12 PASS。待办：渲染层通知中心 UI（恢复/停止）+ preload onRiskHold + 桌面 riskSuspender 接队列派发守卫（端到端随 §7）。 | 本 PR；PRD §12.12 |
