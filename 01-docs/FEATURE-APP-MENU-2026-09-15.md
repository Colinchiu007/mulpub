...
| 服务端主动推送 | 沿用既有 bootstrap 拉取模型。**2026-09-25 定稿**：同步只发生在**应用启动时**（主进程 +3s）；该次同步完成后即广播并让侧边栏自动刷新，**用户不需要任何额外操作，也不需要通过再重启一次来看到配置**（首帧后数秒内自动到位）。运营端保存本身不触达客户端，改动在客户端**下次启动**生效。**应用端界面不提供任何手动同步入口**（详见 §10.1）。 |
...
```
应用端主进程启动（延时 3s）——唯一生效的触发时机；服务方法 syncNow() 仍存在，但界面不提供手动入口（见 §10.1）
...
| 编号 | 位置 | 中文原文 |
|------|------|----------|
| T1 | 页面说明段落 | 管理应用端（桌面端）左侧边栏的菜单项：控制显示 / 隐藏，调整顺序，并可在**「一级导航」与「更多」之间互相拖动**（跨组移动）。「发布、账号、采集、视频创作」为系统核心入口，**强制显示且锁定在一级导航**，不可关闭、不可移出（但可在一级导航内拖动排序）。拖动即自动重排顺序；修改后点「保存」生效。组内排序：把菜单项拖到同组另一项上即可；**跨组移动（一级导航 ↔ 更多）需拖到目标分组的空白处**，拖到具体菜单项上不会跨组。本列表与桌面端菜单目录（应用端 `route-registry`）自动对齐：应用端新增菜单项后，本页会在下次打开时自动补齐该项，**无需点「恢复默认」**；「恢复默认」只用于把显隐与顺序整体回退到目录默认值。<br />生效时机：配置随运行时 bootstrap 下发到应用端。应用端**在启动时同步一次**，因此本页保存的改动会在**客户端下次启动时生效**；那次启动同步完成后侧边栏会自动刷新，用户无需做任何操作。系统不提供服务端主动推送，应用端界面也**不暴露任何同步入口**（运营下发对最终用户透明，不应出现在产品界面里）。 |
...
| # | 局限 | 影响 | 后续建议 |
|---|------|------|----------|
| L1 | 无服务端主动推送：运营端保存不自动触达客户端 | 运营调整有延迟——固定到客户端**下次启动**才生效（该次启动同步在 +3s 自动完成并即时刷新侧边栏，用户无需操作，界面也不提供任何手动入口） | 若延迟不可接受，后续可加 WebSocket 或服务端触发推送；短轮询已在 `design.md` 记为 Rejected（与「界面不出现运营入口」冲突） |
...
| P7 | **文案类修复必须按关键词全仓 sweep，不能逐处改**：P4 首轮只覆盖了当时 grep 到的第一个落点，同一文档另有 4 处（§2 约束表 / §6.2 流程图 / §10 清单 T1 / §14 遗留 L1）+ `ops-center/docs/PRD.md` / `OPERATIONS.md` 各 1 处仍写着旧的「点立即同步」，QM-6 第二轮复评据此判 Critical 复发。收口方式改为：以「立即同步」「手动同步」两个关键词全仓扫 `.md/.vue/.py` 并对**每条命中给出定性**（改 / 判为历史记录 / 判为不相关），改完复扫到 0 命中才收口 | 本文档 §2/§6.2/§10.1/§14 + `ops-center/docs/PRD.md:812` + `OPERATIONS.md:194` + `.quality-gates.md` 复评记录 | ✅ 第二轮补写 |

### 16.6 遗留与部署待办

- **线上 `app_menu_items` 需要该修复部署后才补齐**：部署后必须实测「应用菜单」页出现 `copy-library`（共 20 项），
  且**不要**用点「恢复默认」代替（那会连带重置运营者已有的显隐与顺序）。
- **本次修复不改变"无服务端推送"**：运营端保存要等客户端一次同步才下发，而该同步只发生在**应用启动时**（界面不提供手动入口），故改动在客户端**下次启动**生效。
- **同步入口隐藏后遗留的死代码 / 死键**（本次不删，避免把无关重构混进 Bug 修复）：渲染层 `useOpsCenterSync.runSyncNow` 已无生产调用方（仅自身测试覆盖），`locales/zh.js`+`en.js` 的 `modelProviders.syncNow` 随之成为死键。清理需连带 `useOpsCenterSync.test.js` 的 3 条用例，另立 change。
