...
- 台账 ledger JSON 落 `<userData>/film-production/<taskId>.json`：批/镜双层状态（pending/running/done/failed）、runId、shotIds。
...
| `film-engineering:production-status` | invoke | `{taskId}` | ledger + 批/镜进度（**只读**，不触发生成计费） | 否 |
...
| `film-engineering:production-update` | event 主→渲染 | — | 批次级 running/done/skipped-complete + 逐镜节流进度 | — |
