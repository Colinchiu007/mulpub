...
- **git 陷阱 2（占位符保护必须配对还原）**：为保护功能性依赖 `qianming.yixiaoer.cn`（第三方远程签名服务，百家号签名无本地回退）不被通用规则改坏，先替换成 `qianming.yixiaoer.cn`——但**漏写还原步骤**会让端点在仓库里保持损坏态，且残留扫描发现不了（占位符不含品牌词）。占位符方案必须与还原步骤写在同一脚本里并有断言。
