...
- **侧栏宽度同步（不变）**：`MpSidebar.vue` ResizeObserver → IPC
  `page-manager:set-sidebar-width` → 各 manager `setSidebarWidth`（1–600 合法区间，0 拒绝并回落 200；阈值统一取自 `view-bounds.MIN_SIDEBAR_WIDTH` / `MAX_SIDEBAR_WIDTH`，见 2026-09-15 防御性收口）。
