...
describe('preload 子模块方法数', () => {
  it('publish 模块应导出 117 个键（pipelineConfirmStageGate P2-2 generateAiCover + pipelineCancelRun + P3-7 listPlatformCollections + urlCollectNeedsStealth + renderStartAiVideo）', () => {
    const { createPublishApi } = require('./preload/publish')
    const r = createPublishApi(ipcRenderer)
  ...
  it('合并后 api 总键数应为 316（pipelineConfirmStageGate P2-2 generateAiCover + pipelineCancelRun + P3-7 listPlatformCollections + servicesGetStatus/servicesRestart + urlCollectNeedsStealth + promptLibraryGet/Save/Activate + updateInstallNow + opsCenterSyncAppMenu + onUploadProgress + renderStartAiVideo - webview 分屏监控 API 移除）', () => {
    expect(Object.keys(api).length).toBe(316)
  })

  it('PUBLISH_METHODS 常量包含编排 API', () => {
    expect(PUBLISH_METHODS.length).toBe(82)
