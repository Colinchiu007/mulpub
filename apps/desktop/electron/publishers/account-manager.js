...
async function saveCapturedAccount(platform, captured, options = {}) {
  ...
  const localStorageData =
    source.localStorage &&
    typeof source.localStorage === "object" &&
    !Array.isArray(source.localStorage)
      ? source.localStorage
      : {};
  const indexedDB =
    source.indexedDB && typeof source.indexedDB === "object" && !Array.isArray(source.indexedDB)
      ? source.indexedDB
      : {};
  if (
    cookies.length === 0 &&
    Object.keys(localStorageData).length === 0 &&
    Object.keys(indexedDB).length === 0
  ) {
  ...
  const accountInfo =
    source.accountInfo &&
    typeof source.accountInfo === "object" &&
    !Array.isArray(source.accountInfo)
      ? source.accountInfo
      : {};
  ...
  const credentialSaved = credentialStore.saveCredential(
    accountId,
    {
      platform,
      cookies,
  ...
  if (!credentialSaved) {
    try {
      await pythonBridge.requestBackend("DELETE", `/api/accounts/${accountId}`);
    } catch (_) {
      /* 回滚失败由后端日志记录 */
    }
  ...
  try {
    ...
    if (stateSaved)
      ...
      log.warn(
        "AccountManager",
        `Failed to save account state record for ${platform}:${accountId}`,
      );
  } catch (e) {
...
async function deleteAccount(accountId, options = {}) {
  ...
  try {
    ...
    if (current?.code === 0 && typeof current.data?.platform === "string")
      platform = current.data.platform;
  } catch (_) {
    /* 删除仍可继续，凭证清理使用本地记录回退 */
  }
  ...
  try {
    ...
    if (hadCredential && !deletedCredential) {
      log.warn(
        "AccountManager",
        "加密凭据文件删除失败，账号元数据已删除，凭据文件将在下次清理时重试",
      );
    }
  } catch (e) {
  ...
  try {
    if (
      ownerSubject !== undefined &&
      platform &&
      typeof accountStateRestorer.deleteAccountRecord === "function"
    ) {
...
async function checkLoginStatus(platform, accountId) {
  ...
  const localStorageData =
    ...
    typeof credentials.localStorage === "object" &&
    !Array.isArray(credentials.localStorage)
      ? credentials.localStorage
      : {};
  ...
  if (RENDER_CRASH_PRONE_PLATFORMS.has(platform)) {
    if (cookies.length > 0) {
      ...
      if (httpResult) {
        log.info(
          "AccountManager",
          "checkLoginStatus: render-crash-prone platform " +
            platform +
            ":" +
            accountId +
            " http-check valid=" +
        ...
        if (httpResult.valid === true)
          await refreshProfileFromHttpApi(platform, accountId, cookies);
        return httpResult;
      }
      log.info(
        "AccountManager",
      ...
      log.info(
        ...
        "checkLoginStatus: render-crash-prone platform " +
          ...
          ":" +
          accountId +
          " no cookies (encrypted=" +
          encryptedCookies.length +
          " partition=" +
          partitionCookies.length +
          ")",
      );
    }
    ...
    log.warn(
      "AccountManager",
      "checkLoginStatus: INCONCLUSIVE render-crash-prone without http evidence " +
        platform +
        ":" +
        accountId +
        " cookies=" +
    ...
    return {
      valid: undefined,
      code: "CHECK_LOGIN_INCONCLUSIVE",
      reason: cookies.length > 0 ? "http-check-inconclusive" : "no-cookie",
    };
  }
  ...
  try {
    if (cookies.length === 0 && lsKeys === 0) {
      ...
      log.info(
        "AccountManager",
        "checkLoginStatus: NO_COOKIE_NO_STORAGE " +
          platform +
          ":" +
          accountId +
          " hasAnyCred=" +
    ...
    if (cookies.length === 0 && COOKIE_REQUIRED_PLATFORMS.has(platform)) {
      log.info(
        "AccountManager",
        "checkLoginStatus: NO_COOKIE fast-path " + platform + ":" + accountId + " lsKeys=" + lsKeys,
      );
    ...
    log.info(
      ...
      "checkLoginStatus: start " +
        ...
        " selectors=" +
        (Array.isArray(successSelector)
          ? "[" + successSelector.length + " candidates]"
          : successSelector
            ? "1"
            : "0"),
    );
    ...
    try {
      ...
      if (successSelector) {
        try {
          ...
          log.info(
            "AccountManager",
            "checkLoginStatus: selector timeout " +
              platform +
              ":" +
              accountId +
              " selector=" +
              (Array.isArray(successSelector)
                ? successSelector.join(",").slice(0, 200)
                : String(successSelector).slice(0, 200)) +
              " — 降级 URL 检查",
          );
        }
      }
      ...
      if (currentUrl.includes("login") || currentUrl.includes("signin")) {
        log.info(
          "AccountManager",
          "checkLoginStatus: URL hit login/signin marker " +
            platform +
            ":" +
            accountId +
            " url=" +
      ...
      if (successSelector && !selectorMatched && dashboardUrl) {
        try {
          ...
          if (currentHost === dashboardHost || currentHost.endsWith("." + dashboardHost)) {
            log.info(
              "AccountManager",
              "checkLoginStatus: dashboard-host fallback valid " +
                platform +
                ":" +
                accountId +
                " currentHost=" +
      ...
      log.warn(
        "AccountManager",
        "checkLoginStatus: fallback inconclusive (no selector match, no login URL marker, no dashboard host) " +
          platform +
          ":" +
          accountId +
          " url=" +
    ...
    log.warn(
      "AccountManager",
      "checkLoginStatus: browser-check failed for " +
        platform +
        ":" +
        accountId +
        " error=" +
...
async function refreshProfileFromPage(page, platform, accountId) {
  try {
    ...
    if (!result || result.code !== 0) {
      log.warn(
        "AccountManager",
        "refreshProfileFromPage: 资料回填写入失败 " +
          platform +
          ":" +
          accountId +
          " code=" +
    ...
    log.info(
      "AccountManager",
      "refreshProfileFromPage: 已回填资料字段 " +
        platform +
        ":" +
        accountId +
        " keys=" +
...
async function refreshProfileFromHttpApi(platform, accountId, cookies) {
  try {
    ...
    if (!result || result.code !== 0) {
      log.warn(
        "AccountManager",
        "refreshProfileFromHttpApi: 资料回填写入失败 " +
          platform +
          ":" +
          accountId +
          " code=" +
    ...
    log.info(
      "AccountManager",
      "refreshProfileFromHttpApi: 已回填资料字段 " +
        platform +
        ":" +
        accountId +
        " keys=" +
    ...
    log.warn(
      "AccountManager",
      "refreshProfileFromHttpApi 忽略异常 " +
        platform +
        ":" +
        accountId +
        " err=" +
...
function restoreCookies(session, cookies, baseUrl) {
    ...
    try {
      ...
      return session.cookies
        ...
        .catch((e) => {
          ...
          log.warn(
            "AccountManager",
            "restoreCookies: cookie set failed name=" + (name || "") + " err=" + (e && e.message),
          );
        });
    } catch {
  ...
  return Promise.all(promises).then((results) => {
    if (_restoreFailed > 0)
      log.warn(
        "AccountManager",
        "restoreCookies: " + _restoreFailed + "/" + cookies.length + " cookies failed to restore",
      );
    return results;
  });
}
...
function loadSavedCredentials(accountId, platform, options = {}) {
  ...
  const credentialPlatform =
    typeof credentialData.platform === "string" ? credentialData.platform : "";
...
function setAccountProxy(accountId, platform, proxy, options = {}) {
  if (
    !isSafePathSegment(accountId) ||
    !isSafePathSegment(platform) ||
...
function checkLocalCredentials(platform, accountId, options = {}) {
  ...
  const userDataDir = getUserDataDir();
  const args =
    ownerSubject === undefined ? [accountId, userDataDir] : [accountId, userDataDir, ownerSubject];
  ...
  if (hasEncrypted) {
    ...
    if (!loaded) {
      log.info(
        "AccountManager",
        "checkLocalCredentials: file exists but loadSavedCredentials null for " +
          platform +
          ":" +
          accountId +
          (ownerSubject ? " (owner=" + ownerSubject + ")" : " (legacy)"),
      );
      ...
      if (cookieCount === 0 && !hasPlatformLsSessionMarker(platform, loaded.localStorage)) {
        log.info(
          "AccountManager",
          "checkLocalCredentials: encrypted file has no credential evidence (cookies=0, no LS session marker) " +
            platform +
            ":" +
            accountId +
            " lsKeys=" +
        ...
        log.info(
          "AccountManager",
          "checkLocalCredentials: OK encrypted " +
            platform +
            ":" +
            accountId +
            " cookies=" +
  ...
  if (isSafePathSegment(accountId)) {
    try {
      ...
      for (const sessionCookiePath of sessionCookieCandidates) {
        if (fs.existsSync(sessionCookiePath)) {
          ...
          if (cookieStats.size > 0) {
            log.info(
              "AccountManager",
              "checkLocalCredentials: OK session-cookie " +
                platform +
                ":" +
                accountId +
                " size=" +
                ...
                "B path=" +
                sessionCookiePath +
                " (fallback from missing encrypted file)",
            );
            return true;
          }
          log.info(
            "AccountManager",
            "checkLocalCredentials: session cookie file empty for " +
              platform +
              ":" +
              accountId +
              " path=" +
      ...
      log.warn(
        "AccountManager",
        "checkLocalCredentials: session cookie check error for " +
          platform +
          ":" +
          accountId +
          " " +
  ...
  log.info(
    "AccountManager",
    "checkLocalCredentials: NO credential for " +
      platform +
      ":" +
      accountId +
      " (no encrypted file, no session cookie file)" +
      (ownerSubject ? " (owner=" + ownerSubject + ")" : " (legacy)"),
  );
  return false;
}
...
function _electronSession() {
  try {
    ...
    return electron && electron.session && typeof electron.session.fromPartition === "function"
      ? electron.session
      : null;
  } catch (_) {
    return null;
...
async function getAccountPartitionCookies(platform, accountId) {
  ...
  try {
    ...
    log.warn(
      "AccountManager",
      "getAccountPartitionCookies failed " +
        platform +
        ":" +
        accountId +
        " " +
...
function mergeCookies(primary, extra) {
  ...
  for (const list of [primary, extra]) {
    for (const cookie of Array.isArray(list) ? list : []) {
      const key =
        String((cookie && cookie.name) || "") + "@" + String((cookie && cookie.domain) || "");
...
async function persistLoginState(accountId, platform, status, validatedAt) {
  ...
  if (!ALLOWED.includes(normalized))
    return { ok: false, reason: "invalid-status", status: normalized };
  try {
    ...
    log.info(
      "AccountManager",
      "persistLoginState 固化登录态 " + platform + ":" + accountId + " status=" + normalized,
    );
    return { ok: true, status: normalized };
  } catch (e) {
    log.warn(
      "AccountManager",
      "persistLoginState 异常 " +
        platform +
        ":" +
        accountId +
        " " +
...
async function setAccountActive(accountId, platform, isActive) {
  ...
  try {
    ...
    log.info(
      "AccountManager",
      "setAccountActive 固化启用态 " + platform + ":" + accountId + " is_active=" + isActive,
    );
...
async function updateCapturedAccount(platform, captured, accountId) {
  if (!PLATFORM_LOGIN_URLS[platform]) throw new Error("不支持的平台: " + platform);
  if (!accountId || typeof accountId !== "string" || !isSafePathSegment(accountId))
    throw new Error("缺少或非法账号 ID");
  const ownerSubject = resolveOwnerSubject(undefined);
  const userDataDir = getUserDataDir();
  const source = captured && typeof captured === "object" ? captured : {};
  ...
  try {
    ...
    const stateSaved =
      ownerSubject === undefined
        ...
        : accountStateRestorer.saveAccountRecord(record, ownerSubject, userDataDir);
    if (stateSaved)
      log.info("AccountManager", "Updated account state record for " + platform + ":" + accountId);
    else
      log.warn(
        "AccountManager",
        "Failed to update account state record for " + platform + ":" + accountId,
      );
  } catch (e) {
