...
class UrlCollector {
  ...
  /**
   * 互动计数字符串 → 非负整数（viral-library-integration P0 契约核心）。
   * 支持「1.2万 / 3.4w / 1,234 / 纯数字」；负数/NaN/非数字/超 MAX_SAFE 一律 null——
   * ⚠️ 绝不猜测填 0：0 是强语义（页面明确显示零互动），缺数据必须用 null 表达，
   * 否则爆款分析均值（engagement 0.35 + interaction 0.25 权重）会被缺数条目稀释。
  ...
  _parseEngagement ($, html) {
    ...
    try {
      ...
      $('script[type="application/ld+json"]').each((_, el) => {
        ...
        try {
          ...
          for (const st of arr) {
            ...
            const cnt = UrlCollector._parseEngagementNumber(st && st.userInteractionCount)
      ...
      if (likes === null) {
        ...
        if (m1) likes = UrlCollector._parseEngagementNumber(m1[1])
        else {
          const m2 = html.match(/"like"\s*:\s*(\d+)/)
          if (m2) likes = UrlCollector._parseEngagementNumber(m2[1])
        }
      }
      if (comments === null) {
        const m3 = html.match(/"commentCount"\s*:\s*"?([0-9.,]+[万wW]?)"?/)
        if (m3) comments = UrlCollector._parseEngagementNumber(m3[1])
        else {
          const m4 = html.match(/"reply"\s*:\s*(\d+)/)
          if (m4) comments = UrlCollector._parseEngagementNumber(m4[1])
        }
      }
      ...
      if (likes === null) {
        ...
        if ($vote.length) {
          const parsed = UrlCollector._parseEngagementNumber($vote.text())
