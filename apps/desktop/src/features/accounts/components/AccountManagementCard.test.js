...
describe("AccountManagementCard", () => {
  ...
  it("头像图片加载失败回落占位图标时，失效遮罩仍然显示", async () => {
    const wrapper = mountCard({
      account: { ...account, status: "expired", avatar: "https://cdn.invalid/a.png" },
    });
  ...
  it("遮罩样式契约：头像为定位容器，遮罩绝对定位覆盖并自带半透明底", () => {
    ...
    const vueSrc = fs.readFileSync(
      "./src/features/accounts/components/AccountManagementCard.vue",
      "utf8",
    );
    const avatarRule = vueSrc.slice(
      vueSrc.indexOf(".account-avatar {"),
      vueSrc.indexOf(".account-avatar img"),
    );
  ...
  it("停用账号显示「已停用」标记并灰化卡片，登录徽章不受影响（正交）", () => {
    ...
    expect(wrapper.get('[data-testid="account-card-account-1"]').classes()).toContain(
      "is-disabled",
    );
  ...
  it("is_active 缺失或为脏值时按启用处理，不显示停用标记", () => {
    for (const candidate of [{}, { is_active: true }, { is_active: "no" }, { is_active: 0 }]) {
      ...
      expect(wrapper.get('[data-testid="account-card-account-1"]').classes()).not.toContain(
        "is-disabled",
      );
    }
  });
  ...
  it("异常状态显示异常徽章并在非法检查时间后回退失败原因", () => {
    ...
    expect(wrapper.get('[data-testid="account-check-account-1"]').text()).toBe(
      "异常：Cookie 已过期",
    );
  });
  ...
  it("按参考产品卡片语义展示粉丝数、负责人、运营人和代理字段", () => {
    ...
    expect(wrapper.get('[data-testid="account-followers-account-1"]').text()).toContain(
      "粉丝：2048",
    );
    ...
    expect(wrapper.get('[data-testid="account-proxy-account-1"]').text()).toContain(
      "127.0.0.1:7890",
    );
  });
  ...
  it("归属徽章按参考产品契约分色：负责人蓝、运营人灰、代理紫", () => {
    ...
    const kinds = new Set(
      [ownerBadge.classes(), publisherBadge.classes(), proxyBadge.classes()]
        .flat()
...
describe("账号卡片显示修复（PRD-ACCOUNT-CARD-DISPLAY-FIX-2026-09-24）", () => {
  it("顶部平台名 chip 显示平台名而非账号名", () => {
    const wrapper = mountCard({
      account: {
        ...
        platform: "kuaishou",
        status: "active",
        account_name: "0粉丝0关注0获赞账号认证退出登录命运石",
      },
      platformLabel: "快手",
    });
  ...
  it("噪声账号名回落平台名，真实昵称不受影响", () => {
    const dirty = mountCard({
      account: { id: "account-1", platform: "douyin", status: "active", account_name: "作品发布" },
      platformLabel: "抖音",
    });
  ...
  it("已检测账号（last_validated）不再显示「暂无检查记录」", () => {
    const wrapper = mountCard({
      account: {
        ...
        status: "active",
        account_name: "数字丘丘",
        last_validated: "2026-09-24T05:39:39.040Z",
      },
    });
