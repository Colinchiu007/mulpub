/**
 * YixiaoerSidebar 运营中心「应用菜单」配置生效测试（2026-09-15）
 *
 * 覆盖：
...
vi.mock('@/stores/identity', () => ({
  useIdentityStore: () => ({ status: 'authenticated', user: null, displayName: '', entitlement: null }),
}))

vi.mock('@/stores/license', () => ({
  useLicenseStore: () => ({ isPro: true, isTrial: false, isFree: false }),
}))
...
function primaryKeys (w) {
  return w.findAll('a.yixiaoer-primary-item').map((el) =>
    el.attributes('data-testid').replace('yixiaoer-primary-', ''),
  )
}

/** 「更多」折叠菜单项的 href（= 路由路径），用于校验排序 */
function moreHrefs (w) {
  return w.findAll('a.yixiaoer-more-item').map((el) => el.attributes('href'))
}
...
describe('YixiaoerSidebar — 应用菜单配置生效', () => {
  it('未下发配置（data=null）→ fail-open，渲染完整默认菜单与默认顺序', async () => {
    ...
    expect(sidebar.find('[data-testid="yixiaoer-primary-more"]').exists()).toBe(true)
  })
  ...
  it('「更多」组全部隐藏 → 触发器按钮与折叠菜单都不渲染', async () => {
    ...
    expect(sidebar.find('[data-testid="yixiaoer-primary-more"]').exists()).toBe(false)
