<template>
  <aside class="mp-sidebar" data-testid="mp-sidebar" aria-label="主导航">
    <header class="mp-sidebar-header">
      <span class="mp-sidebar-brand" aria-hidden="true">MP</span>
      <strong class="mp-sidebar-title">Multi-Publish</strong>
    ...
    <nav class="mp-primary-nav" aria-label="主导航">
      ...
      <div v-if="moreOpen" class="mp-more-menu" role="menu">
...
<style scoped>
.mp-sidebar {
  width: var(--mp-sidebar-width, 200px);
  min-width: var(--mp-sidebar-width, 200px);
  flex-shrink: 0;
  display: flex;
...
.mp-sidebar-add:hover,
.mp-sidebar-add:focus-visible {
  border-color: #5149e8;
  color: #5149e8;
}
...
.mp-primary-item:hover,
.mp-primary-item:focus-visible {
  background: rgba(255, 255, 255, .65);
  color: #5149e8;
}

.mp-primary-item.active {
...
.mp-more-trigger > svg:last-child {
  width: 13px;
  height: 13px;
  margin-left: auto;
...
.mp-more-trigger > svg:last-child.rotated {
...
@media (max-width: 900px) {
  ...
  .mp-sidebar-title,
  .mp-sidebar-add,
  .mp-primary-item span,
  .mp-primary-item > svg:last-child,
  .mp-more-menu,
  .mp-sidebar-service {
    display: none;
  }
