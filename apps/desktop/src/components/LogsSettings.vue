<template>
  <div class="logs-settings">
    ...
    <!-- Network & scheduler diagnostics (black-box, production-visible, no params) -->
...
async function runNetSchedDiagnose () {
  ...
  try {
    ...
    if (!eapi || typeof eapi.rateLimitSelfCheck !== 'function') {
      diagnoseResult.value = { level: 'fail', text: t('settings.diagnose.fail') }
      return
    }
    const res = await eapi.rateLimitSelfCheck({ rpm: 20, requestCount: 6, requestDurationMs: 80 })
