          ...
          <el-table-column label="校验规则" min-width="320">
            <template #default="{ row }">
              <div v-for="rule in row.rules" :key="rule.rule" style="display:flex;gap:8px;align-items:center;padding:2px 0">
                ...
                <span v-if="!rule.pass" style="color:var(--color-danger);font-size: var(--font-size-xs)">实际={{ rule.actual }}（期望 {{ rule.expected }}）</span>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
<style scoped>
.hint { color: var(--color-text-secondary); font-size: var(--font-size-xs); }
.metric-row { margin-bottom: 8px; }
.metric-card { text-align: center; }
.metric-label { font-size: var(--font-size-xs); color: var(--color-text-secondary); }
.metric-value { font-size: var(--font-size-lg); font-weight: 600; margin-top: 4px; }
.json-pre { background: var(--color-bg-inset); padding: 12px; border-radius: 6px; font-size: var(--font-size-xs); max-height: 240px; overflow: auto; }
</style>
