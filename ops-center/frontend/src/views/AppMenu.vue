<template>
  <div>
    ...
    <p style="color:var(--color-text-placeholder);margin-bottom:16px;font-size: var(--font-size-sm)">
      ...
      本列表与桌面端菜单目录（应用端 <code>route-registry</code>）自动对齐：应用端新增菜单项后，本页会在下次打开时自动补齐该项，<strong>无需点「恢复默认」</strong>；
      「恢复默认」只用于把显隐与顺序整体回退到目录默认值。
      <br />
      生效时机：配置随运行时 bootstrap 下发到应用端。应用端<strong>在启动时同步一次</strong>，
      因此本页保存的改动会在<strong>客户端下次启动时生效</strong>；那次启动同步完成后侧边栏会自动刷新，
      用户无需做任何操作。系统不提供服务端主动推送，应用端界面也<strong>不暴露任何同步入口</strong>
      （运营下发对最终用户透明，不应出现在产品界面里）。
    </p>
