...
async def _provision_from_catalog(db: AsyncSession) -> None:
    ...
    """
    rows = (await db.execute(sa.select(AppMenuItem.item_key))).scalars().all()
    existing = set(rows)
    missing = [(index, entry) for index, entry in enumerate(CATALOG) if entry[0] not in existing]
    if not missing:
        return
    now = _now()
    for index, (key, label, group, description) in missing:
        db.add(
            AppMenuItem(
                item_key=key,
                label=label,
