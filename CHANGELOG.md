...
### 新增
- **身份链路诊断日志**：`AuthService._logFailure()` 记录 `scope + code + cause 链`，埋点覆盖 `signIn`/`signInCleanup`/`tokenStorage.clear`/`clearLocalSession`/`clearSignInWindowSession`/`getAccessToken`(.network/.sessionRejected)/`restore`/`signOut.remote`；logger 由工厂注入（`options.logger` 可覆盖）
- i18n 新增 `memberCenter` 15 组词条（zh/en 成对）：`retryHint`/`loginFailed`/`loginCancelled`/`loginInProgress`/`loginTimeout`/`loginCallbackFailed`/`loginWindowFailed`/`operationInProgress`/`operationFailed`/`identityServiceUnavailable`/`identityLoadFailed`/`networkUnavailable`/`switchAccountRequired`/`sessionInvalid`/`sessionStoreBlocked`/`secureStorageUnavailable`
...
- `vitest run`（secure-token-storage / auth-service / identity-service-factory / stores·identity / ProfileMenu / identityErrorMessages / MemberCenter）→ **7 files / 111 passed**（新增 19 例：存储降级与重试 5、主错误保留与日志 3、错误码映射 5、ProfileMenu 文案 4、store cleanup 保真 2）
